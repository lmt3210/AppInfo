...
APP_ENTITLEMENTS="AppInfo.entitlements"
APP=./AppInfo.xcarchive/Products/Applications/AppInfo.app
...
if [ "${ACTION}" == "package" ]; then
    ...
    codesign -s "${DEV_ID_APP}" -f --timestamp -o runtime \
        --entitlements "${APP_ENTITLEMENTS}" tmp_app/AppInfo.app

    # Build component package
    pkgbuild --root tmp_app --identifier com.larrymtaylor.AppInfo \
    ...
    productbuild --sign "${DEV_ID_INST}" --distribution dist.plist \
        --product requirements.plist AppInfo.pkg
    ...
    xcrun notarytool submit AppInfo.pkg --keychain-profile \
        ${DEV_KEY_PROF} --wait

    # Staple notary ticket to product
    xcrun stapler staple AppInfo.pkg
